...
These tests build labeled synthetic inputs with known ground truth and report
precision / recall, turning "it detects near-duplicates" into a measured number.
"""
...
def _make_embeddings(seed: int = 0):
    """Build 64-d embeddings: 5 near-duplicate groups (2-3 frames each) + 10 distinct
    singletons. Returns (vectors, ground_truth_group_id_per_vector)."""
    rng = np.random.default_rng(seed)
    vectors: list[np.ndarray] = []
    groups: list[int] = []

    # Near-duplicate groups: a base vector plus near-identical copies (tiny noise).
    group_sizes = [3, 2, 2, 3, 2]
    for gid, size in enumerate(group_sizes):
        base = rng.standard_normal(64)
        for _ in range(size):
            vectors.append(base + 0.01 * rng.standard_normal(64))
            groups.append(gid)

    # Distinct singletons: each its own group id, no near-duplicate partner.
    for s in range(10):
        vectors.append(rng.standard_normal(64))
        groups.append(100 + s)
...
def test_near_duplicate_precision_recall():
    ...
    # eps=0.05 should perfectly separate near-identical frames from distinct shots
    # on this well-separated synthetic set — no false merges, no missed duplicates.
